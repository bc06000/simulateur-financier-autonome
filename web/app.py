...
DOSSIER_STATS = RACINE_PROJET / "sauvegardes"
FICHIER_VISITES = DOSSIER_STATS / "statistiques_visites.csv"
ENTETES_VISITES = ["date", "heure", "ip", "methode", "page", "navigateur"]


def _initialiser_fichier_visites():
    DOSSIER_STATS.mkdir(parents=True, exist_ok=True)

    if not FICHIER_VISITES.exists() or FICHIER_VISITES.stat().st_size == 0:
        with FICHIER_VISITES.open("w", encoding="utf-8-sig", newline="") as fichier:
            writer = csv.writer(fichier, delimiter=";")
            writer.writerow(ENTETES_VISITES)
...
def _enregistrer_visite():
    if request.path == "/statistiques/telecharger":
        return

    _initialiser_fichier_visites()
    maintenant = datetime.now()

    with FICHIER_VISITES.open("a", encoding="utf-8-sig", newline="") as fichier:
        writer = csv.writer(fichier, delimiter=";")
        writer.writerow([
            maintenant.strftime("%d/%m/%Y"),
            maintenant.strftime("%H:%M:%S"),
            _obtenir_ip_visiteur(),
            request.method,
            request.path,
...
def enregistrer_visite():
    try:
        ...
        print(f"Erreur journal statistiques : {exc}")


@app.route("/statistiques/telecharger")
def telecharger_statistiques():
    _initialiser_fichier_visites()
    contenu = FICHIER_VISITES.read_bytes()

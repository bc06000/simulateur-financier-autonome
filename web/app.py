...
def _origine_visite():
    ...
    if "facebook.com" in referer or "fbclid" in request.args:
        return "Facebook"
...
def telecharger_statistiques():
    ...
    if FICHIER_VISITES.exists():
        contenu = FICHIER_VISITES.read_text(
            encoding="utf-8-sig"
        )
    else:
        contenu = (
            ";".join(ENTETES_VISITES)
            + "\n"
        )
    ...
    return Response(
        ...
        headers={
            "Content-Disposition":
            "attachment; filename=statistiques_visites.csv"
        },
    )
...
def construire_projection_annuelle(
    simulation,
):
    lignes = []
    ...
    for numero, ligne_historique in enumerate(
        ...
        capital = float(
            ligne_historique["capital"]
        )

        versements = (
            simulation.capital_initial
            + simulation.versement_mensuel
            * (numero * 12)
        )
...
def construire_points_graphique(
    simulation,
):
    ...
    for index, capital in enumerate(
        capitaux
    ):
...
.orbite{
    ...
    border-radius:50%;
    box-shadow:
        0 0 25px rgba(55,232,255,.16),
        inset 0 0 30px rgba(55,232,255,.08);
}
...
.r1{transform:rotate(205deg)}
.r2{transform:rotate(335deg)}
...
<div class="orbite"></div>

<div class="coeur"></div>

<div class="rayon r1"></div>
<div class="rayon r2"></div>
<div class="rayon r3"></div>
<div class="rayon r4"></div>
...
<div class="valeur">
{{ capital_final if simulation else "0 €" }}
</div>
...
<div class="valeur">
{{ risque if simulation else "En attente" }}
</div>
...
<div class="valeur">
{{ score_ia if simulation else "0.0 / 10" }}
</div>
...
<div class="valeur">
{{ performance if simulation else "+0.0 %" }}
</div>
...
<div class="valeur">
{{ profil_ia if simulation else "En attente" }}
</div>
...
<div class="valeur">
{{ duree_resultat if simulation else "En attente" }}
</div>
...
<form
    ...
    class="
        formulaire
        {% if request.method == 'POST' and erreur %}
        ouvert
        {% endif %}
    "
    method="post"
...
<input
    ...
    min="0"
    step="10"
    value="{{ valeurs.versement_mensuel }}"
    required
>
...
Outil pédagogique de simulation —
aucune projection ne constitue une garantie
de résultat futur.

<br>

© 2026 Simulateur Financier Autonome —
...
def accueil():
    ...
    service_projection = (
        obtenir_service_projection_utilisateur()
    )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            service_projection.ajouter_simulation(
                simulation
            )
    ...
    if simulation:
        ...
        projection_annuelle = (
            construire_projection_annuelle(
                simulation
            )
        )

        graphique_points = (
            construire_points_graphique(
                simulation
            )
        )

        capital_final = formater_euros(
            simulation.capital_final
        )

        total_versements = formater_euros(
            simulation.total_versements
        )

        gains = formater_euros(
            simulation.gains
        )

        performance = (
            f"{simulation.performance:.2f} %"
        )
        ...
        rendement_resultat = (
            f"{simulation.taux:.2f} %"
        )

        risque = calculer_risque(
            simulation
        )
...
.synthese-grille strong{
    display:block;
    font-size:12px;
    margin-bottom:8px
}
.synthese-grille span{
    font-size:12px
}
.copyright{
    text-align:center;
...
<div class="sous">
Comparez deux simulations enregistrées
</div>

{% if simulations|length < 2 %}

<div class="scenario vide">
Au moins 2 simulations sont requises
...
<option
    value="{{ loop.index0 }}"
    {% if loop.index0 == index_a %}
    selected
    {% endif %}
>
{{ loop.index }}
-
{{ item.date_creation.strftime('%d/%m/%Y %H:%M') }}
-
{{ euros(item.capital_final) }}
</option>
...
{% for titre, sim in [
    ('SCÉNARIO A', simulation_a),
    ('SCÉNARIO B', simulation_b)
] %}
...
<div class="ligne">
<strong>Capital initial</strong>
<span>{{ euros(sim.capital_initial) }}</span>
</div>

<div class="ligne">
<strong>Versement mensuel</strong>
<span>{{ euros(sim.versement_mensuel) }}</span>
</div>

<div class="ligne">
<strong>Rendement annuel</strong>
<span>{{ "%.2f"|format(sim.taux) }} %</span>
</div>
...
<span>{{ euros(sim.total_versements) }}</span>
</div>

<div class="ligne">
<strong>Capital final</strong>
<span>{{ euros(sim.capital_final) }}</span>
</div>

<div class="ligne">
<strong>Plus-value</strong>
<span>{{ euros(sim.gains) }}</span>
</div>

<div class="ligne">
<strong>Performance</strong>
<span>{{ "%.2f"|format(sim.performance) }} %</span>
</div>
...
<div>
<strong>Meilleur capital final</strong>
<span>{{ meilleur_capital }}</span>
</div>

<div>
<strong>Meilleure plus-value</strong>
<span>{{ meilleure_plus_value }}</span>
</div>

<div>
<strong>Meilleure performance</strong>
<span>{{ meilleure_performance }}</span>
</div>

<div>
<strong>Écart de capital final</strong>
<span>{{ ecart_capital }}</span>
</div>
...
def comparaison():
    ...
    simulations = list(
        service_projection.obtenir_simulations()
    )
    ...
    simulation_a = simulations[
        index_a
    ]

    simulation_b = simulations[
        index_b
    ]

    if (
        simulation_a.capital_final
        >= simulation_b.capital_final
    ):
    ...
    if (
        simulation_a.performance
        >= simulation_b.performance
    ):

        meilleure_performance = (
            "Scénario A — "
            f"{simulation_a.performance:.2f} %"
        )
...
<h1>
Simulateur financier autonome
</h1>

<p>
Simulez • Projetez • Décidez
</p>
...
<td>
{{ sim.date_creation.strftime('%d/%m/%Y %H:%M') }}
</td>

<td>
{{ euros(sim.capital_initial) }}
</td>

<td>
{{ euros(sim.versement_mensuel) }}
</td>
...
<td>
{{ euros(sim.total_versements) }}
</td>

<td>
{{ euros(sim.capital_final) }}
</td>
...
<div class="intro">
Une question ou une remarque concernant le simulateur ?
</div>
...
<div class="info">
Le bouton ouvre le logiciel de messagerie
